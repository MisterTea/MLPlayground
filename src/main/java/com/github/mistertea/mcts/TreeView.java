...
public class TreeView extends JComponent {
	...
	public TreeView(AbstractTreeNode root) {
        this.root = root;
	...
	private void draw(Graphics2D g, AbstractTreeNode cur, int x, int y, int wFac) {
        // draw this one, then it's children

        int arity = cur.arity();
        for (int i = 0; i < arity; i++) {
			if (cur.children.get(i).nVisits > 0) {
	...
	private void drawNode(Graphics2D g, AbstractTreeNode node, int x, int y) {
        String s = (int) node.totValue + "/" + (int) node.nVisits;
		if (node.getTerminalValue() != null) {
			s = "F: " + String.format("%.2f", node.getTerminalValue());
		}
        g.setColor(nodeBg);
